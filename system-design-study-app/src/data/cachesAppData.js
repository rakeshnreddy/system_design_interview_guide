...
export const cachesAppData = {
  overview: "Caching is a fundamental performance optimization technique that involves storing copies of frequently accessed data in a temporary, high-speed storage layer (a `{{Cache}}`). This `{{Cache}}` is strategically placed closer to the requesting client—be it a user's browser, an application server, or another service—than the original data source (e.g., a `[database](#/databases)` or a remote `{{API (Application Programming Interface)|API}}`). The primary goals of caching are to significantly speed up data retrieval operations, thereby reducing `{{Latency}}`, and to lessen the load on the origin data store, which improves overall system `{{Scalability}}` and responsiveness. When an application or client requests data, the system first checks the `{{Cache}}`. If the requested data is found in the `{{Cache}}` (a '`{{Cache Hit}}`'), it is returned immediately, leading to a fast response. If the data is not found in the `{{Cache}}` (a '`{{Cache Miss}}`'), the system must then fetch it from the slower, origin data store. Once retrieved, this data is typically stored in the `{{Cache}}` for subsequent requests, turning future identical requests into cache hits. This mechanism is vital for minimizing repetitive and resource-intensive operations such as complex `{{Database|database}}` queries, external `{{API (Application Programming Interface)|API}}` calls, or computationally expensive rendering tasks. While effective caching strategies are crucial for building fast, `{{Scalability|scalable}}`, and cost-efficient systems, they also introduce complexities. Key challenges include maintaining data `{{Consistency}}` between the `{{Cache}}` and the origin store (addressed by `{{Cache Invalidation}}` strategies and ensuring `{{Cache Coherency}}`), determining what data is most valuable to `{{Cache|cache}}` (see `{{Cache Entry}}`), and deciding how long it should remain cached (see `{{Time To Live (TTL)}}`).\n\n_Q: What eviction policy would you choose if your access pattern is mostly scans through data (e.g., iterating over all items once)?_\n_A: For scan-heavy workloads, {{FIFO (First-In, First-Out)|FIFO}} might outperform {{LRU (Least Recently Used)|LRU}}. LRU would quickly fill with scanned items, evicting potentially useful older data that might be needed again. FIFO would cycle through items regardless of recent access, which is more aligned with a scan pattern._\n\n_Q: When is using a {{Write-Around}} caching strategy more beneficial than {{Write-Through}}?_\n_A: {{Write-Around}} is better when data is written but not always read immediately or frequently. It avoids filling the {{Cache}} with data that might not be needed, saving space for more popular read items. {{Write-Through}} is good for consistency but can pollute the cache with write-only or rarely-read data._",
  mermaidDiagrams: {
    cacheAside: `
flowchart LR
    A[Client] --> B{Cache?}
...
`
  ...
  metrics: [
    ...
    { id: "miss-rate", name: "{{Cache Miss Rate|Miss Rate / Miss Ratio}}", description: "Percentage of requests not found in cache, requiring origin fetch.", talk: "Analyze misses to improve cache content.",
    ...
    { id: "eviction-rate", name: "{{Cache Eviction Rate|Eviction Rate}}", description: "Rate at which items are removed from cache due to space or TTL.", talk: "Monitor evictions to tune cache size/policies.",
  ...
  terminology: [
    ...
    { term: "{{Cache Coherency}}", definition: "Ensures that all clients or nodes in a `{{Distributed Caches|distributed system}}` have a consistent view of the data stored in caches. It addresses challenges that arise when multiple caches store the same data, and that data is updated. For example, if one application instance updates its local cache and a distributed cache, other instances relying on their own local caches or different replicas of the distributed cache might see stale data if coherency mechanisms (like `{{Cache Invalidation|cache invalidation protocols}}` or `{{Consensus Algorithm|consensus algorithms}}`) are not in place. See also `[database replication concepts](#/databases?section=replication)`." },
  ...
  cachepedia: {
    "{{Client-Side Caching}}": {
      ...
      cons: [
        ...
        "Data `{{Consistency}}` can be a major issue; `{{Cache Invalidation|cache invalidation}}` is hard to enforce from the server.",
    ...
    "{{CDN (Content Delivery Network)}}": {
      ...
      cons: [
        ...
        "`{{Time To Live (TTL)|TTL (Time-To-Live)}}` management is crucial; too long TTLs cause staleness, too short reduce hit rate.",
    ...
    "{{In-Memory Caches (Local)}}": {
      ...
      whenToUse: "Frequently accessed data that is specific to a single application instance (e.g., user `{{Session Affinity|session data}}` in a non-distributed setup). Small, relatively static datasets that can fit comfortably in memory. Reducing redundant computations or data lookups within a single request lifecycle.",
    ...
    "{{Distributed Caches}}": {
      ...
      pros: [
        "Highly `{{Scalability|scalable}}`, can store large amounts of data by adding more cache servers.",
        "Shared cache accessible by multiple application instances or `{{Microservices Architecture|microservices}}`, ensuring data `{{Consistency}}`.",
      ...
      cons: [
        ...
        "Can become a `{{Single Point of Failure (SPOF)|single point of failure}}` if not configured for `{{High Availability|high availability}}` (e.g., clustering, replication).",
      ...
      whenToUse: "Sharing application data or session state across multiple instances of an application or different `{{Microservices Architecture|microservices}}`. Offloading read load from primary `{{Database|databases}}` for frequently accessed data. When cache size requirements exceed the RAM of a single application server. Implementing features like `{{Rate Limiting}}`, leaderboards, or real-time messaging.",
  ...
  writePatterns: [
    {
      ...
      cons: [
        ...
        "`{{Cache}}` might store data that is not frequently read (write-once, read-never), leading to inefficient use of cache memory unless combined with `{{Time To Live (TTL)|TTLs}}`."
      ],
...
`,
      ...
      cons: [
        ...
        "Data in the origin is `{{Eventual Consistency|Eventually Consistent}}`, which might not be acceptable for all applications."
      ],
  ...
  scenarios: {
    "{{E-commerce Product Page Caching}}": {
      ...
      solution: { strategy: "Multi-level cache: `{{CDN (Content Delivery Network)}}` for `{{Static Assets}}` (images, CSS), `{{Distributed Caches}}` for product details (`{{Read-Through Cache Aside}}`), local `{{In-Memory Caches (Local)}}` for hot products. `{{TTLs (Time-To-Live)|TTLs}}` for pricing/stock.", components: ["CDN", "`[Redis](#/databases?section=redis)`/`[Memcached](#/databases?section=memcached)`", "Local In-Memory Cache"] }
    },
    "{{News Feed Caching}}": {
  ...
  decisionTree: {
    ...
    nodes: {
      ...
      "latencyDataSharing": {
        ...
        options: [
          { answer: "Yes, data needs to be shared.", recommendation: "`{{Distributed Caches}}` (e.g., `[Redis](#/databases?section=redis)`, `[Memcached](#/databases?section=memcached)`). Consider factors like data size, `{{Persistence}}` needs, and specific features like `{{Pub/Sub}}` or data structures." },
      ...
      "originLoadContentType": {
        ...
        options: [
            ...
            { answer: "Dynamic (frequently changes, personalized)", recommendation: "`{{Distributed Caches}}` (e.g., `[Redis](#/databases?section=redis)`, `[Memcached](#/databases?section=memcached)`) combined with Application-Level Caching Strategies. Consider `{{Cache Invalidation}}` techniques carefully." },
      ...
      "mixedContentStrategy": {
        ...
        options: [
            { answer: "Highly separable (e.g., static shell, dynamic content loaded via API)", recommendation: "Use `{{CDN (Content Delivery Network)}}` for static assets and `{{Distributed Caches}}` for API responses/dynamic data components." },
            { answer: "Tightly coupled", recommendation: "Focus on `{{Distributed Caches}}` with effective application-level caching and `{{Cache Invalidation}}`. A `{{CDN (Content Delivery Network)}}` might still be used for the main entry point or less dynamic portions if applicable." }
        ]
      }
    }
  },
  ...
  keyTakeaways: [
    ...
    "Manage data consistency carefully: Implement effective `{{Cache Invalidation}}` strategies (e.g., `{{Time To Live (TTL)}}`, `{{Write-Through Cache|write-through}}`, event-based) and ensure `{{Cache Coherency}}` in distributed setups to prevent stale data issues. Link to `[database replication concepts](#/databases?section=replication)` for more on consistency.",
    ...
    "Be aware of patterns and pitfalls: Understand caching patterns like `{{Write-Through}}`, `{{Write-Back (Write-Behind)}}`, and `{{Cache-Aside (Lazy Loading)|Cache-Aside}}`. Mitigate issues like the `{{Thundering Herd}}` effect with strategies such as `{{Request Coalescing}}`."
  ]
};