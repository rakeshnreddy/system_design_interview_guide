export const scalabilityConceptsAppData = {
  ...
  overview: "{{Scalability}} in system design aims to ensure a system can handle growth in users, data volume, and transaction rates efficiently. This involves making trade-offs between different scaling strategies, such as {{Vertical Scaling (Scaling Up)|vertical scaling}} (increasing resources of a single server) versus {{Horizontal Scaling (Scaling Out)|horizontal scaling}} (adding more servers). Key considerations include maintaining {{Fault Tolerance}} (the system's ability to operate despite failures) and managing the increased complexity that often accompanies more distributed architectures. The goal is to achieve {{Performance}}, {{Availability}}, and {{Cost-Effectiveness}} at scale.\n\n_Q: What's the difference between {{Vertical Scaling}} and {{Horizontal Scaling}}?_\n_A: {{Vertical Scaling}} (scaling up) involves increasing the resources of a single server (e.g., adding more CPU, RAM). {{Horizontal Scaling}} (scaling out) involves adding more servers to distribute the load. Horizontal scaling is generally preferred for high {{Availability}} and massive {{Scalability}}._\n\n_Q: Why is {{Statelessness}} beneficial for scalability?_\n_A: {{Statelessness|Stateless applications}} do not store client session data on the server that handled the request. This means any server can handle any request, making it much easier to distribute load across many servers and to add or remove servers dynamically without losing session information. State is typically managed externally (e.g., in a distributed {{Cache}} or {{Database}})._",
  metrics: [
    {
