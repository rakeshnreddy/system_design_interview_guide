...
const FundamentalsView = ({ appData }) => {
  ...
  const keyTakeaways = appData.keyTakeaways || [];
    ...
    <div className="space-y-8 p-1">
      <div className="prose max-w-none dark:prose-invert">
        ...
        <p className="text-lg text-neutral-700 dark:text-neutral-300 leading-relaxed">
          <RenderTextWithLinks text={appData.overview || introText} glossaryData={glossaryData} />
        </p>
      </div>
        ...
        <Card padding="p-6">
          <div>
            <h2 className="text-3xl font-bold text-neutral-800 dark:text-neutral-100 mb-4">
              <RenderTextWithLinks text={cacheCoherencyTerm.term} glossaryData={glossaryData} />
            </h2>
            <div className="prose prose-lg dark:prose-invert max-w-none text-neutral-700 dark:text-neutral-300">
              <p><RenderTextWithLinks text={cacheCoherencyTerm.definition} glossaryData={glossaryData} /></p>
              <p className="mt-2">
                <strong>Example Challenge:</strong> Consider a distributed caching system with multiple nodes (e.g., Redis cluster) fronting a sharded [database](#/databases). If a user updates their profile information, the change is written to a specific database shard and its corresponding cache node (Cache Node A). However, if another part of the system reads the same user's profile from a different cache node (Cache Node B) before Node A's update has propagated or Node B has been invalidated, it will serve stale data. Ensuring that Node B becomes aware of the change (e.g., through invalidation messages, a pub/sub system, or a consistent hashing redirect to the updated node) is a key aspect of cache coherency. This becomes even more complex with multi-level caching (e.g., client-side cache, CDN, server-side distributed cache). For more on data consistency across systems, see [database replication concepts](#/databases?section=replication).
              </p>
            </div>
          </div>
        </Card>
      ...
      {keyTakeaways.length > 0 && (
        <Card padding="p-6">
          <div>
            <h2 className="text-3xl font-bold text-neutral-800 dark:text-neutral-100 mb-4">Key Takeaways</h2>
            <ul className="list-disc space-y-2 pl-5 prose prose-lg dark:prose-invert max-w-none text-neutral-700 dark:text-neutral-300">
