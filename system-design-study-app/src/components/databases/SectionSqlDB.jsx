    ...
    return (
        <div className={`bg-neutral-50 dark:bg-neutral-700/50 ${lastItem ? 'rounded-b-lg' : ''} ${idx === 0 && !isOpen ? 'rounded-t-lg' : ''} ${isOpen && idx === 0 ? 'rounded-t-lg' : ''}`}>
            <button
                ...
                className={`w-full flex justify-between items-center p-5 text-left font-semibold text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-600/50 transition-colors
                ...
                `}
            >
                <span className="text-lg">{title}</span> {/* Accordion title style */}
                {/* Replace with actual SVG icon for chevron */}
